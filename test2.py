...
if subprocess.call(['msbuild', 'build\\win32\\zlib.sln', '/p:Configuration=Release', '/p:Platform=Win32']) == 0:
...
if subprocess.call(['msbuild', 'build\\win32\\zlib.sln', '/p:Configuration=Release', '/p:Platform=x64']) == 0:
    # Копируем файлы в отдельную директорию
...
if os.path.exists(os.path.join(output_dir, 'zlib.lib')) and os.path.exists(os.path.join(output_dir, 'zlib.dll')):
